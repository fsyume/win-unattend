...
⚠️ **清单里只允许 ASCII 路径**。cmd.exe 按系统 OEM 代码页解析，路径含中文或空格会生成乱码
文件名甚至下载失败。当前 `tool` 里有 **2 个文件因此没进清单**——其中一个是 418 MB 的
`QI-ANXING Tianqing(...平衡防御版).exe`。**建议改名后重新生成清单**。

⚠️ **只要增删了 `tool` 里的文件（包括新增 `start.bat`）就必须重新生成清单**，
否则客户端会漏文件（日志会打印 `MISSING or EMPTY`）。改名命令、生成命令和全部细节见
