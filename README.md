...
| `drvceo_files.txt` | 本仓库提供，原样复制（驱动包的文件清单，44 行） |
| `tool\Drvceo_Win10_Win11_x64_Lite\` | 驱动总裁整包，约 40 MB |

**为什么是逐文件下载**：iVentoy 的 HTTP 是静态文件服务，**只能按文件取，不能取目录，也没有目录列表**。
所以清单在服务器侧预先列好，脚本按清单一个个拉回来、保持目录结构，最终还原到客户机的 `C:\DrvCeo\`。

⚠️ **改了驱动包内容就必须重新生成清单**，否则客户端会漏文件（日志会打印 `MISSING or EMPTY`）。
生成命令和全部细节见 `user/deploy/README.md`。
